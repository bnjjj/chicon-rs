...
impl MemFileSystem {
    ...
    fn get_from_relative_path(&self, path: PathBuf) -> Option<MemDirEntry> { //TODO: prefer return reference
        ...
        let child = if let Some(child_entry) = self.children.get(&current_path.to_string_lossy().into_owned()) {
            child_entry
        } else {
            println!("làààà");
            return None;
        ...
        println!("bla {:?} --> {:?}", self.children.keys(), &current_path.to_string_lossy().into_owned());
        child.get_from_relative_path(path_iter.collect())
    }

    fn insert_file(&mut self, path: PathBuf) -> Result<MemFile, ChiconError> { //TODO: prefer return reference
        let mut path_iter = path.iter();
        let current_path = path_iter.next().ok_or(ChiconError::BadPath)?;

        // if something already exist 
        if let Some(entry) = self.children.get_mut(&current_path.to_string_lossy().into_owned()) {
            match entry {
                MemDirEntry::Directory(dir) => dir.insert_file(path_iter.collect()),
                MemDirEntry::File(file) => {
                    file.truncate(path_iter.collect())?;
                    Ok(file.clone()) //TODO: prefer return reference
                },
            }
        } else {
            // create file
            if path_iter.clone().peekable().peek().is_some() {
                ...
                self.children.insert(file.name.clone(), MemDirEntry::File(file.clone()));
            
                Ok(file) // Won't work because not a reference
            } else { // insert directory and call insert_file on it
                let mut dir = MemDirectory {
                    name: current_path.to_string_lossy().into_owned(),
                    perm: Permissions::from_mode(0o755),
                    children: None,
                };
                let file_created = dir.insert_file(path_iter.collect())?;
                self.children.insert(dir.name.clone(), MemDirEntry::Directory(dir.clone()));

                Ok(file_created) // Won't work because not a reference
            }
        }
    }
}
...
impl MemFile {
    fn truncate(&mut self, path: PathBuf) -> Result<(), ChiconError> {
        if !self.content.is_empty() { // check with std::fs library if it clears the content
            self.content.clear();
        }
        Ok(())
    }
...
impl DirEntry for MemDirEntry {
    ...
    fn path(&self) -> Result<PathBuf, Self::FSError> {
        match self {
            ...
            MemDirEntry::File(file) => Ok(PathBuf::from(file.name.clone())),
        }
    }
...
impl MemDirectory {
    fn get_from_relative_path(&self, path: PathBuf) -> Option<MemDirEntry> { //TODO: prefer return reference
        ...
        children.get(&current_path.to_string_lossy().into_owned()).and_then(|entry| entry.get_from_relative_path(path_iter.collect()))
    }
...
mod tests {
    ...
    fn test_fs_internals() {
        ...
        mem_fs.children.insert(String::from("test"), MemDirEntry::Directory(dir));
